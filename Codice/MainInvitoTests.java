...
public class MainInvitoTests {
    public static void main(String[] args) {
        ...
        // Service
        InvitoService invitoService = new InvitoService(invitoRepository, teamRepository, userRepository,
        ...
        // Leader role updated via Constructor or Logic, but for test setup we can
        // re-create or assume service does it.
        // Here we simulate the state AFTER team creation, so leader should already
        // appear as leader.
        ...
        // Updating team leader reference to the new object (though technically ID ref
        // is enough for repos)
        // team = new Team("Alpha Team", leader); // Re-creating team with updated
        // leader if needed, but entity holds ref.

        User memberUpdated = new User(member.getId(), member.getNome(), member.getCognome(), member.getEmail(),
                Ruolo.MEMBRO_TEAM);
        ...
        System.out
                .println("Team created: " + team.getNomeTeam() + " with Leader: " + team.getLeaderSquadra().getNome());

        // 3. Test: Send Invite
        System.out.println("\n[TEST 1] Send Invite...");
        try {
            CreaInvitoRequest req = new CreaInvitoRequest(team.getId(), target.getEmail(), leader.getId());
        ...
        try {
            ...
            CreaInvitoRequest reqInv = new CreaInvitoRequest(team.getId(), target3.getEmail(), leader.getId());
        ...
        try {
            // Create Hackathon
            Hackathon hackathon = new Hackathon("Hackathon 2024", "Regole...", LocalDateTime.now(),
                    LocalDateTime.now().plusDays(10), LocalDateTime.now().plusDays(20),
                    LocalDateTime.now().plusDays(30), "Roma", 1000, 5, leader, leader, new ArrayList<>(),
            ...
            CreaInvitoRequest req = new CreaInvitoRequest(team.getId(), target2.getEmail(), leader.getId());
            Invito invito = invitoService.inviaInvito(req);
